...
// — ENVIAR PESQUISA PARA WEBHOOK (Google Sheets / Make / Zapier) ————
async function enviarPesquisa(dados) {
  if (!SHEETS_WEBHOOK) {
    console.log('[Webhook] Sem URL configurada — pulando envio da pesquisa');
    return;
  }
  try {
    const payload = {
...
function montarMapaFrontend(hd, planetas, portoes, canais) {
  ...
  return {
    ...
    tipo: {
      ...
      frase: tipoObj ? tipoObj.fraseIdentidade : '',
    },
    estrategia: { label: tr(estrategiaRaw), desc: getDesc('estrategia', estrategiaRaw) },
    autoridade: { label: tr(autoridadeRaw), desc: getDesc('autoridade', autoridadeRaw) },
    perfil: { label: perfilRaw, desc: getDesc('perfil', perfilRaw) },
    definicao: { label: tr(definicaoRaw), desc: getDesc('definicao', definicaoRaw) },
    assinatura: { label: tr(assinaturaRaw), desc: getDesc('assinatura', assinaturaRaw) },
    desalinhamento: { label: tr(naoSelfRaw), desc: getDesc('naoself', naoSelfRaw) },
...
if (email) {
  ...
  console.log('[2] Sem email — pula confirmacao');
}
